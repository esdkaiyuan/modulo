...
若视频含音轨,会**同步自动取模**:输出区域在帧数组之后附带 PCM 音频数组(`*_audio` / `*_audio_len` / `*_audio_rate`),以及波形/频谱点阵;可在侧栏调整采样率、位深与是否写入 PCM。
...
把图片转成可照图摆豆的拼豆图纸，三种工作区：图片取模、手绘编辑、自由排版。

- **图片取模**：按品牌色卡匹配色号，支持等比 / 锁定宽高比 / 自由尺寸，可**排除指定颜色**（重新匹配到最接近的剩余色），支持 AI 抠图（U2-Net）与边缘/容差去背景
- **手绘编辑**：画笔 / 橡皮 / 填充 / 取色，撤销重做，可修改画布尺寸（保留左上角内容）
- **自由排版**：导入图片、加入已生成的图纸、添加 emoji / 符号贴纸，支持拖拽、旋转、缩放、图层与撤销重做，整体取样为豆子网格
- **物料清单**：色号、色名、数量、占比，可按数量 / 色号 / 名称排序并一键复制；换算拼豆板数量
- **导出**：PNG / JPEG / PDF 打印（A4 横向含物料表）/ 物料 CSV / **逐格坐标 CSV** / JSON / **C 数组头文件**（RGB565 调色板 + 每格索引字节，可直接驱动点阵屏）
